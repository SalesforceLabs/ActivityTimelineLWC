...
import getDateFilterOptions from '@salesforce/apex/RecordTimelineDataProvider.getDateFilterOptions';
...
export default class ActivityTimeline extends LightningElement {
    ...
    @track dateFilterSelection;
    
    @track isLoading = true;
    @track isLoadingTimeLineFilter = false;
    @track serverData;
    @track searchText;
        ...
        for (let [key, value] of Object.entries(groupedByMonth)) {
            ...
            /*if (Date.parse(key) - new Date().getTime() > 0 ) {
                ...
                if(tasksAndChildRecords.length>0){
                    monthItem.monthValue = moment(key);
